...
function addSource(instance, source, content) {
    ...
    } else if (typeof source !== 'number' || source < 0 || source >= sources.length || ! isFinite(source)) {
        throw new TypeError('source must be string or integer in sources range');
    }
    ...
    if (arguments.length > 2) {
        contents[index] = content === null ? null : String(content);
    }
...
function insert(instance, index, item) {
    ...
    index = normalizeIndex(index, instance.length, 0);
