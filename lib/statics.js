...
function insert(instance, index, item) {
    ...
    if (hasPoints(item)) {
        ...
        push.apply(args, asimilate(instance, item));
    } else {
...
function append(instance, items) {
    ...
    for (i = 0, l = items.length; i < l; i++) {
        ...
        if (hasPoints(item)) {
            ...
            push.apply(points, asimilate(instance, item));
        } else {
            content += item;
            points.length = content.length;
        }
...
function prepend(instance, items) {
    ...
    for (i = items.length; i--;) {
        ...
        if (hasPoints(item)) {
            ...
            unshift.apply(points, asimilate(instance, item));
        } else {
            content = item + content;
...
function asimilate(instance, target) {
    var i, l, point;
    ...
    for (i = 0, l = points.length; i < l; i++) {
        ...
        result[i] = point && [
            point[0] == null ? null : sourcesMap[point[0]],
            point[1] == null ? null : point[1],
            point[2] == null ? null : point[2],
            point[3] == null ? null : namesMap[point[3]],
        ];
    }
...
function clone(instance) {
    ...
    util.define(copy, 'length', 0);
    util.define(copy, 'content', instance.content);
    util.define(copy, 'points', instance.points.slice());
    util.define(copy, '$$lines', []);
