// TODO: Move mappings to a saparate project
...
function optimize(mappings) {
    ...
    sources = sources.filter(isFinite);
    names = names.filter(isFinite);

    if (sl !== sources.length || nl !== names.length) {
...
function _find(mappings, needle, compareSort, compare, lower) {
    var index = sortedIndex(mappings, needle, compareSort);
